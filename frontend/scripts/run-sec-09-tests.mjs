...
const workdir = resolve(tmpdir(), "mitchabrim-sec09-emulator");
const configHome = resolve(tmpdir(), "mitchabrim-sec09-firebase-config");
...
writeFileSync(resolve(workdir, "firebase.json"), JSON.stringify({ firestore: { rules: "firestore.rules" }, emulators: { auth: { port: 9799 }, firestore: { port: 8780 }, ui: { enabled: false }, singleProjectMode: true } }));
...
const result = spawnSync(process.execPath, [cli, "emulators:exec", "--project", "demo-sec09", "--only", "auth,firestore", `node ${JSON.stringify(resolve(root, "tests", "sec-09.test.mjs"))}`], {
