...
const workdir = resolve(tmpdir(), "mitchabrim-sec08-emulator");
const configHome = resolve(tmpdir(), "mitchabrim-sec08-firebase-config");
...
writeFileSync(resolve(workdir, "firebase.json"), JSON.stringify({
  ...
  emulators: { auth: { port: 9699 }, firestore: { port: 8680 }, ui: { enabled: false }, singleProjectMode: true },
}));
...
const result = spawnSync(process.execPath, [cli, "emulators:exec", "--project", "demo-sec08", "--only", "auth,firestore", `node ${JSON.stringify(resolve(root, "tests", "sec-08.rules.test.mjs"))}`], {
