...
export default function Gallery() {
  ...
  useEffect(() => {
    const fetchAlbums = async () => {
      try {
        // Get only public images (isPublic === true) and type image
        const q = query(
        ...
        // Group images by folder name or category
        const albumsMap = {};
        images.forEach((img) => {
          // Use folderName if available, otherwise use category, fallback to "ללא קטגוריה"
          const category = img.folderName || img.category || "ללא קטגוריה";
          if (!albumsMap[category]) {
            albumsMap[category] = [];
          }
          albumsMap[category].push(img);
        });

        // Convert to array with counts and cover images
        const albumsArray = Object.entries(albumsMap)
          .map(([name, images]) => ({
            name,
            images,
            count: images.length,
      ...
      <div className="container" style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 20px" }}>
        ...
        {albums.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 20px", color: "#888" }}>
            ...
            <p>עדיין לא הועלו תמונות ציבוריות למערכת. חזרו מאוחר יותר.</p>
          </div>
        ) : (
          <div style={{ 
            display: "grid", 
